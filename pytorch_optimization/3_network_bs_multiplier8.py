...
class Trainer:
    def __init__(self, args, model):
        ...
        self.trainloader = torch.utils.data.DataLoader(
            trainset, batch_size=args.batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_memory)

        testset = torchvision.datasets.CIFAR10(
            root='./data', train=False, download=True, transform=transform_test)
        self.testloader = torch.utils.data.DataLoader(
            testset, batch_size=args.batch_size, shuffle=False, num_workers=num_workers, pin_memory=pin_memory)
...
if __name__ == "__main__":
    ...
    model = [ResNet18(), ResNet50(), ResNet101(), MobileNetV2(),\
        Rec_ResNet18(), Rec_ResNet50(), Rec_ResNet101(), Rec_MobileNetV2()]
    batch_size = [128, 128, 128, 128, 129, 129, 129, 129]
    for m, bs in zip(model, batch_size):
        args.batch_size = bs
