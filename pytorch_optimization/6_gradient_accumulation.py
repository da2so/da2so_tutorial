...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            if self.ga:
                if (batch_idx +1) % 2 == 0 or (batch_idx+1) == len(self.trainloader):
                    self.optimizer.step()
                    self.optimizer.zero_grad()
            else:
                self.optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gradient_accum', default=True, type=bool, help='set gradient to None')
    parser.add_argument('--epochs', default=5, type=int, help='epochs')
    args = parser.parse_args()
    
    
    gradient_accum = [False, True]
    model = [ResNet18(), ResNet50(), ResNet101(), MobileNetV2()]
    for m in model:
        for ga in gradient_accum:
            args.gradient_accum = ga
